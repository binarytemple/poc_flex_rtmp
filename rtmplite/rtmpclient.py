...
class Client(Protocol):
    ...
    def handshake(
            self):  # Implement the client side of the handshake. Must be invoked by caller after TCP connection completes.
    ...
    def send(self, cmd,
             timeout=None):  # Call a RPC method on the server. This is used for connect, createStream, publish, etc.
    ...
    def messageReceived(self, msg):  # invoked by base class framework to handle a receive message.
        if (msg.type == Message.RPC or msg.type == Message.RPC3) and msg.streamId == 0:
            ...
            yield self.queue.put(cmd,
                                 timeout=5)  # RPC call, must be processed by application within 5 seconds, or will be discarded.
...
class NetConnection(object):
    ...
    def __init__(self):
        ...
        self.data = Object(videoCodecs=252.0, audioCodecs=3191.0, flashVer='WIN 10,0,32,18', swfUrl=None,
                           videoFunction=1.0, capabilities=15.0, fpad=False, objectEncoding=0.0)
    ...
    def connect(self, url, timeout=None, *args):  # Generator to connect to the given url, and return True or False.
        ...
        try:
            ...
            result, fault = yield self.client.send(Command(name='connect', cmdData=self.data, args=args),
                                                   timeout=timeout)
...
class Result(
    Exception): pass  # a result class is used to send result of copy operation from within a nested control flow.


class Resource(
    object):  # base class for different types of resources. Just defines an internal queue to get and put messages.
    __slots__ = ['url', 'type', 'mode']

    def __init__(self): self.url = self.type = self.mode = None; self.queue = multitask.SmartQueue()

    def get(self, timeout=None, criteria=None): result = yield self.queue.get(timeout=timeout,
                                                                              criteria=criteria); raise StopIteration, result
...
class RTMPReader(Resource):  # connect to RTMP URL and play the stream identified by id in URL.
    ...
    def open(self, url):
        self.url, options = url, dict(
            map(lambda x: tuple(x.split('=', 1) + [''])[:2], url[7:].partition('?')[2].split('&')))
        self.timeout, self.stream = int(options['timeout']) if 'timeout' in options else 10, options[
            'id'] if 'id' in options else None
        if not self.stream:
    ...
    def run(self):
        try:
            while True:
                msg = yield self.ns.stream.queue.get(timeout=self.timeout, criteria=lambda x: x is None or x.type in (
...
def open(url, mode='r'):
    ...
    types = {'rtmp-r': RTMPReader, 'rtmp-w': RTMPWriter, 'http-r': HTTPReader, 'http-w': HTTPWriter,
             'file-r': FLVReader, 'file-w': FLVWriter}
...
def connect(url, params=None, timeout=10, duration=60, publishStream='publish', playStream='play', publishFile=None,
            enableAudio=True, enableVideo=True):
    ...
    '''
    if _debug: print 'connect url=%r params=%r timeout=%r duration=%r publishStream=%r playStream=%r publishFile=%r enableAudio=%r enableVideo=%r' % (
        url, params, timeout, duration, publishStream, playStream, publishFile, enableAudio, enableVideo)
...
if __name__ == '__main__':
    ...
    try:
        ...
        if _debug: print 'keyboar interrupt'
